...
import com.dhcc.sso.exception.CheckUserInfoErrorException;
...
public class CustomHandlerAuthentication extends AbstractPreAndPostProcessingAuthenticationHandler {
    ...
    protected AuthenticationHandlerExecutionResult doAuthentication(Credential credential) throws GeneralSecurityException, PreventedException {
        ...
        User info;
        try {
            info = jdbcTemplate.queryForObject(sql, new Object[]{username}, new BeanPropertyRowMapper<>(User.class));
        } catch (EmptyResultDataAccessException e) {
            info = null;
        }

        if (info == null) {
            throw new CheckUserInfoErrorException();
        }
